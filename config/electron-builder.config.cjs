...
const devChannelRepo = isMacHourly
  ...
  : isMacDaily
    ...
    : isMacAdhoc
      ? 'logicmantra-adhoc'
      : null
const appId = 'com.logicmantra.ide'
...
module.exports = {
  ...
  nsis: {
    artifactName: 'logicmantra-windows-setup.${ext}',
  ...
  dmg: {
    artifactName: 'logicmantra-macos-${arch}.${ext}'
  },
  linux: {
    // Why: Ubuntu desktop ships GNOME Orca as the `orca` package and /usr/bin/orca.
    // The Linux installer should not claim those system package/file names.
    executableName: 'logicmantra-ide',
    ...
    desktop: {
      entry: {
        // Why: Electron reports WM_CLASS from the app name; GNOME docks need an
        // exact match to group the window with logicmantra-ide.desktop.
        StartupWMClass: 'logicmantra-ide'
      }
    },
    extraResources: [
      ...commonExtraResources,
    ...
    target: ['AppImage', 'deb'],
    maintainer: 'logicmantra',
    category: 'Utility'
  },
  appImage: {
    artifactName: isLinuxArm64Release ? 'logicmantra-linux-arm64.${ext}' : 'logicmantra-linux.${ext}'
  },
  deb: {
    packageName: 'logicmantra-ide',
    artifactName: 'logicmantra-ide_${version}_${arch}.${ext}',
  ...
  rpm: {
    packageName: 'logicmantra-ide',
    artifactName: 'logicmantra-ide-${version}.${arch}.${ext}',
  ...
  publish: {
    provider: 'github',
    owner: 'logicmantra',
    repo: devChannelRepo ?? 'logicmantra',
