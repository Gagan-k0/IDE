...
5. **Integrate the OmniRoute AI gateway** into the IDE as a Settings pane (mirrors the Baton integration): probe omniroute/node/Claude availability, start/stop/monitor the gateway server on port 20128 (runs detached via `--daemon`, no terminal window), run `omniroute setup-claude` to generate Claude Code profiles per model, auto-configure the default `claude` terminal command to route through OmniRoute on first start, and open the dashboard in the built-in browser tab. DONE 2026-08-15 on branch `feature/omniroute` (commits `fd5eabd` + `028a009`, pushed to `origin/feature/omniroute`, NOT merged to main). See "OmniRoute integration notes" below.
...
- CLI: default command is `serve` (starts the Next.js server). `omniroute serve --no-open --port 20128` = non-interactive, no browser pop. `omniroute setup-claude` fetches the LIVE /v1/models catalog from the running server and writes ~/.claude/profiles/<name>/settings.json for each model (idempotent, reuses setup-codex profile names). Requires the server to be running first (the IDE's configureClaude starts it automatically). `omniroute launch --profile <name>` injects ANTHROPIC_AUTH_TOKEN at runtime (token never written to disk). `omniroute serve --daemon` spawns the server detached (stdio ignore + unref) and the CLI exits immediately - the IDE uses `--daemon --no-open --port 20128` so NO terminal window stays open (user requirement 2026-08-15).
...
- First-time setup (DONE 2026-08-15): clicking Start OmniRoute with Claude Code available and NOT already configured auto-runs `omniroute:configureClaude` - it starts the server if needed, runs setup-claude (per-model profiles), then WRITES/MERGES the DEFAULT ~/.claude/settings.json env block so the plain `claude` terminal command routes through OmniRoute (ANTHROPIC_BASE_URL=http://127.0.0.1:20128, ANTHROPIC_AUTH_TOKEN=omniroute-no-auth sentinel - mirrors omniroute launch so newer Claude Code skips its login gate against an open backend, ANTHROPIC_MODEL=auto, CLAUDE_CODE_ENABLE_GATEWAY_MODEL_DISCOVERY=1, CLAUDE_CODE_AUTO_COMPACT_WINDOW=190000). Backs up the original to settings.json.omniroute-backup first; merges, never drops unrelated user settings; BOM-safe read (JSON.parse rejects UTF-8 BOM). Idempotent - re-running skips the write when already configured. Works on ANY machine (no hardcoded paths; uses homedir()/resolveCliCommand).
...
- Pattern used (identical shape to Baton): src/shared/omniroute-types.ts (OmniRouteAvailability/OmniRouteSetupResult/OmniRouteClaudeConfig/OmniRouteConfigureClaudeResult/OmniRouteServerStatus + OMNIROUTE_PORT=20128/OMNIROUTE_URL/OMNIROUTE_HEALTH_URL); src/main/ipc/omniroute-server-process.ts (reuses captureCommandOutput/killDaemonPid/pidListeningOnPort from baton-daemon-process.ts via re-export, adds isOmniRouteReachable/probeOmniRouteHealth/waitForOmniRouteReady(60s)/waitForOmniRouteGone(15s)); src/main/ipc/omniroute-server.ts (lifecycle: get/start/stop, reuse-if-running, taskkill tree on win32, spawns `serve --daemon --no-open --port 20128` detached+windowsHide so no terminal opens); src/main/ipc/omniroute.ts (registerOmniRouteHandlers: getAvailability/getServerStatus/startServer/stopServer/getClaudeConfig/configureClaude; configureClaude supersedes the removed standalone setupClaude IPC); registered at register-core-handlers.ts right after registerBatonHandlers().
...
- Pane behavior: no folder dependency (gateway is global, unlike Baton). Buttons Start OmniRoute (startServer + createBrowserTab to dashboard; auto-configures Claude Code on first start when claude available + not configured), Open Dashboard (needs server.running + activeWorktreeId), Configure Claude Code (configureClaude - starts server if needed + setup-claude + writes default settings.json; no longer requires server.running), Stop. Status shows CLI v/Node/Claude Code availability + server running url/version/pid + Claude Code configured-for-OmniRoute indicator.
