...
function expectFinalZdotdirRestoreContext(content: string) {
  expect(content).toContain("after LogicMantra's last wrapper file has loaded")
  expect(content).toContain('export ZDOTDIR="$_orca_home"')
}
...
describePosix('daemon shell-ready launch config', () => {
  ...
  it('uses inherited ORCA_ORIG_ZDOTDIR when ZDOTDIR is a LogicMantra wrapper dir', async () => {
    const previousZdotdir = process.env.ZDOTDIR
  ...
  itWithBash(
    'dispatches a non-empty preexec_functions against the real command, not LogicMantra hooks',
    async () => {
      const { getDaemonBashShellReadyRcfileContent } = await importFreshShellReady()
