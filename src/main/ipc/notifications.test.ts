...
describe('registerNotificationHandlers', () => {
  ...
  it('suppresses active-worktree notifications while LogicMantra is focused', async () => {
    getAllWindowsMock.mockReturnValue([
      {
        isDestroyed: () => false,
        isFocused: () => true
  ...
  it('uses the macOS default notification sound when no custom sound is configured', async () => {
    ...
    try {
      ...
      expect(notificationCtorMock).toHaveBeenCalledWith({
        title: 'LogicMantra notifications are on',
        body: 'This is a test notification from LogicMantra.',
        sound: 'default'
      })
    } finally {
      Object.defineProperty(process, 'platform', { value: originalPlatform, configurable: true })
...
describe('triggerStartupNotificationRegistration', () => {
  ...
  it('shows welcome notification when not yet requested', async () => {
    ...
    expect(store.updateUI).toHaveBeenCalledWith({ notificationPermissionRequested: true })
    expect(notificationCtorMock).toHaveBeenCalledWith({
      title: 'LogicMantra is ready to notify you',
      body: 'Allow notifications so LogicMantra can alert you when agents finish or terminals need attention.'
    })
    expect(notificationShowMock).toHaveBeenCalledTimes(1)
  })
