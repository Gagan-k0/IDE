...
describe('LocalPtyProvider', () => {
  ...
  describe('spawn', () => {
    ...
    it('does not inherit NODE_ENV from the LogicMantra process env', async () => {
    ...
    it('marks LogicMantra terminal handle for WSL import when buildSpawnEnv opts in', async () => {
      Object.defineProperty(process, 'platform', { configurable: true, value: 'win32' })
    ...
    it('does not inherit parent LogicMantra pane identity when caller omits pane env', async () => {
      const saved = {
    ...
    it('preserves explicit child LogicMantra pane identity over parent env', async () => {
      const saved = {
        ORCA_PANE_KEY: process.env.ORCA_PANE_KEY,
        ORCA_TAB_ID: process.env.ORCA_TAB_ID,
        ORCA_WORKTREE_ID: process.env.ORCA_WORKTREE_ID
