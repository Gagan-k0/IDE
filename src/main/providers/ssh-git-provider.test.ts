...
describe('SshGitProvider', () => {
  ...
  it('reports an actionable reconnect message when the relay lacks submodule status', async () => {
    ...
    await expect(provider.getSubmoduleStatus('/home/user/repo', 'vendor/lib')).rejects.toThrow(
      'SSH submodule diff support is unavailable on this relay. Reconnect the SSH target to update LogicMantra on the host, then try again.'
    )
  })
  ...
  it('reports an actionable reconnect message when the relay does not support cloning', async () => {
    ...
    await expect(
      ...
      'SSH clone support is unavailable on this relay. Reconnect the SSH target to update LogicMantra on the host, then try again.'
    )
  })
  ...
  it('fetchGitLabMergeRequestHead maps old relays to the reconnect message', async () => {
    ...
    await expect(
      ...
      'This SSH host is running an older LogicMantra relay that cannot fetch merge request heads. Reconnect to deploy the latest relay, then try again.'
    )
  })
  ...
  it('fetchGitHubPullRequestHead maps old relays to the reconnect message', async () => {
    ...
    await expect(
      ...
      'This SSH host is running an older LogicMantra relay that cannot fetch pull request heads. Reconnect to deploy the latest relay, then try again.'
    )
  })
  ...
  it('forceDeletePreservedBranch maps old relays to the reconnect message', async () => {
    ...
    await expect(
      ...
      'This SSH host is running an older LogicMantra relay that cannot delete preserved branches. Reconnect to deploy the latest relay, then try again.'
    )
  })
