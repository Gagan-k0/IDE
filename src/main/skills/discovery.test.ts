...
describe('skill discovery', () => {
  ...
  it('discovers skill packages through symlinked skill directories', async () => {
    ...
    await writeFile(join(realSkill, 'SKILL.md'), '# LogicMantra CLI\n\nUse the LogicMantra CLI.')
    ...
    const skill = result.skills.find((entry) => entry.name === 'LogicMantra CLI')
  ...
  it('keeps home classification when cwd points at the same directory as home', async () => {
    ...
    await writeFile(
      ...
      ['---', 'name: orca-cli', 'description: Use the LogicMantra CLI.', '---', ''].join('\n')
    )
