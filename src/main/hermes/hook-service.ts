...
const HERMES_PLUGIN_MARKER = 'Managed by LogicMantra. Do not edit; changes may be overwritten.'
...
function getPluginFilesState(pluginDir = getPluginDir()): {
  ...
  try {
    ...
    return {
      present: true,
      managed,
      detail: managed ? null : 'Hermes orca-status plugin exists but is not LogicMantra-managed'
    }
  } catch (error) {
    return {
      present: true,
...
function getPluginManifest(): string {
  return [
    ...
    'version: 1.0.0',
    'description: "Reports Hermes Agent lifecycle events to LogicMantra."',
    'author: "LogicMantra"',
    'kind: standalone',
    'provides_hooks:',
