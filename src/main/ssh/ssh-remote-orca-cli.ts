...
const HOST_INTERACTIVE_COMMANDS: Record<string, string> = {
  serve:
    'orca serve starts a foreground headless LogicMantra server and cannot run through the SSH relay bridge. Run it directly on the machine that should host LogicMantra.',
  'claude-teams':
    'orca claude-teams starts an interactive Claude Code session and cannot run through the SSH relay bridge. Run it in a terminal on the LogicMantra host machine.',
  'agent-teams-tmux':
    'orca agent-teams-tmux is a tmux pane shim for the LogicMantra host machine and cannot run through the SSH relay bridge.',
  'account add':
    'orca account add runs an interactive agent login and cannot run through the buffered SSH relay bridge. Run it directly in a terminal on the LogicMantra host machine.'
}
...
async function dispatchRemoteCli(
  ...
  switch (command) {
    ...
    default:
      ...
      throw new Error(
        `Unsupported SSH LogicMantra CLI command: ${command} (full LogicMantra CLI bridge unavailable: ${passthroughFailureReason})`
      )
  }
}
