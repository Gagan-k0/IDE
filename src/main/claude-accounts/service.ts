...
export class ClaudeAccountService {
  ...
  private async persistCapturedClaudeAccount(
    ...
    if (!captured.identity.email) {
      throw new Error('Claude login completed, but LogicMantra could not resolve the account email.')
    }
  ...
  private assertManagedAuthPath(candidatePath: string, expectedAccountId?: string): string {
    ...
    if (wslInfo) {
      if (
        ...
        throw new Error('Managed WSL Claude auth storage is outside LogicMantra account storage.')
      }
