...
describe('registerAppMenu', () => {
  ...
  it('routes Check for Updates modifier clicks to prerelease and perf checks', () => {
    ...
    // either way.
    const parentLabel = isMac ? 'LogicMantra' : 'Help'
  ...
  it.each(['darwin', 'linux', 'win32'] as const)(
    'routes Edit > Paste through LogicMantra coordinated paste ownership on %s',
    (platform) => {
  ...
  it.each(['darwin', 'linux', 'win32'] as const)(
    'routes Edit selection actions through the focused LogicMantra window on %s',
    (platform) => {
      vi.spyOn(process, 'platform', 'get').mockReturnValue(platform)
      const send = vi.fn()
      getFocusedWindowMock.mockReturnValue({ webContents: { send } })
  ...
  it.runIf(!isMac)('puts Settings and Exit under File on Windows/Linux', () => {
    ...
    expect(template.find((item) => item.label === 'LogicMantra')).toBeUndefined()
    ...
    expect(helpLabels).toEqual(
      expect.arrayContaining([
        'Report Crash...',
        'Getting Started with LogicMantra',
        'Explore LogicMantra',
        'Check for Updates...'
      ])
    )
  ...
  it.runIf(isMac)('keeps the macOS app-named menu with Settings and quit roles', () => {
    ...
    const template = getTemplate()
    const appSubmenu = getSubmenu(template, 'LogicMantra')
  ...
  it('routes Getting Started with LogicMantra through its callback', () => {
    const options = buildMenuOptions()
    registerAppMenu(options)

    const setupGuideItem = getSubmenu(getTemplate(), 'Help').find(
      (entry) => entry.label === 'Getting Started with LogicMantra'
    )
  ...
  it('routes Feature tour through its callback', () => {
    ...
    const featureTourItem = getSubmenu(getTemplate(), 'Help').find(
      (entry) => entry.label === 'Explore LogicMantra'
    )
  ...
  it('exposes an Appearance submenu under View with checkbox items reflecting state', () => {
    ...
    const mobileItem = appearanceSubmenu.find((item) => item.label === 'Show LogicMantra Mobile Button')
  ...
  it('routes Appearance checkbox clicks through onToggleAppearance', () => {
    ...
    appearanceSubmenu
      ...
      ?.click?.({} as never, {} as never, {} as never)
    appearanceSubmenu
      .find((item) => item.label === 'Show LogicMantra Mobile Button')
      ?.click?.({} as never, {} as never, {} as never)
    appearanceSubmenu
