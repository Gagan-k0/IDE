...
function makeHostStatus(
  launcherPath = 'C:\\Users\\me\\AppData\\Local\\Programs\\LogicMantra\\resources\\bin\\orca.exe'
) {
  return {
    platform: 'win32',
    commandName: 'orca',
    commandPath: launcherPath,
    pathDirectory: 'C:\\Users\\me\\AppData\\Local\\Programs\\LogicMantra\\resources\\bin',
    pathConfigured: true,
    launcherPath,
...
ORCA_WIN_LAUNCHER='C:\\Program Files\\LogicMantra\\resources\\bin\\orca.cmd'
...
if command -v powershell.exe >/dev/null 2>&1; then
  ...
  echo "LogicMantra WSL CLI requires Windows interop and could not find powershell.exe." >&2
  exit 1
fi
  ...
  const runner = vi.fn(async (_distro: string, command: string) => {
    ...
    if (command.includes('cat > "$command_tmp"')) {
      ...
      if (
        ...
        !files.get(bridgePath)?.includes('# LogicMantra managed WSL CLI PowerShell bridge')
      ) {
      ...
      if (files.get(legacyCommandPath)?.includes('# LogicMantra managed WSL CLI launcher')) {
    ...
    if (command.includes('rm -f')) {
      ...
      if (
        ...
        files.get(legacyCommandPath)?.includes('# LogicMantra managed WSL CLI launcher')
      ) {
        files.delete(legacyCommandPath)
      }
      return ''
...
describe('WslCliInstaller', () => {
  ...
  it('installs a WSL launcher that forwards to the Windows LogicMantra launcher', async () => {
    ...
    expect(wsl.getFile()).toBe(
      _internals.buildWslLauncher(
        'C:\\Users\\me\\AppData\\Local\\Programs\\LogicMantra\\resources\\bin\\orca.exe',
  ...
  it('continues checking WSL when the host launcher exists but host PATH is unknown', async () => {
    ...
    const hostStatus = {
      ...makeHostStatus(),
      pathConfigured: null,
      detail: 'LogicMantra could not read the Windows user PATH registry value.'
    } satisfies CliInstallStatus
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
  ...
  it('reports installed WSL launchers whose bin directory is missing from PATH', async () => {
    ...
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
      hostInstaller: { getStatus: async () => makeHostStatus('C:\\LogicMantra\\orca.cmd') },
      wslRunner: wsl.runner
    })
  ...
  it('accepts current managed WSL scripts with an extra heredoc trailing newline', async () => {
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'installed',
      currentTarget: 'C:\\LogicMantra\\orca.cmd'
    })
  })
  ...
  it('generates a launcher that forwards arguments through a PowerShell file bridge', () => {
    const launcher = _internals.buildWslLauncher(
      'C:\\Program Files\\LogicMantra\\resources\\bin\\orca.exe',
      '/home/alice/.local/share/orca/orca-wsl-bridge.ps1'
    )
    ...
    expect(launcher).toContain(
      'LogicMantra WSL CLI requires Windows interop and could not find powershell.exe.'
    )
  ...
  it('marks stale managed launchers that point at the old app bin instead of packaged resources', async () => {
    const oldLauncher = _internals.buildWslLauncher(
      'C:\\Users\\me\\AppData\\Local\\Programs\\LogicMantra\\bin\\orca.cmd',
      '/home/alice/.local/share/orca/orca-wsl-bridge.ps1'
    )
    ...
    await expect(installer.getStatus()).resolves.toMatchObject({
      state: 'stale',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\LogicMantra\\bin\\orca.cmd',
      launcherPath: 'C:\\Users\\me\\AppData\\Local\\Programs\\LogicMantra\\resources\\bin\\orca.exe'
    })

    await expect(installer.install()).resolves.toMatchObject({
      state: 'installed',
      currentTarget: 'C:\\Users\\me\\AppData\\Local\\Programs\\LogicMantra\\resources\\bin\\orca.exe'
    })
  })

  it('repairs the frozen pre-rc4 registration so orchestration send/reply reach native rc4', async () => {
    const nativeLauncher = 'C:\\Program Files\\LogicMantra\\resources\\bin\\orca.exe'
  ...
  it('repairs a managed launcher whose bridge is missing, but preserves a conflicting bridge', async () => {
    const nativeLauncher = 'C:\\LogicMantra\\resources\\bin\\orca.exe'
    ...
    const staleBridge = createWslRunner(PRE_RC4_MANAGED_WSL_LAUNCHER, true, {
      initialBridge: '# LogicMantra managed WSL CLI PowerShell bridge\nWrite-Output "stale"\n'
    })
  ...
  it('keeps the pre-rc4 files on a transactional replacement failure', async () => {
    ...
    const installer = new WslCliInstaller({
      ...
      distro: 'Ubuntu',
      hostInstaller: {
        getStatus: async () => makeHostStatus('C:\\Program Files\\LogicMantra\\resources\\bin\\orca.exe')
      },
      wslRunner: wsl.runner
    })
  ...
  it('is idempotent after repairing an old managed registration', async () => {
    const nativeLauncher = 'D:\\Custom LogicMantra\\resources\\bin\\orca.exe'
    const wsl = createWslRunner(PRE_RC4_MANAGED_WSL_LAUNCHER)
    const installer = new WslCliInstaller({
      platform: 'win32',
      distro: 'Ubuntu',
    ...
    expect(wsl.getFile()).toContain("ORCA_WIN_LAUNCHER='D:\\Custom LogicMantra\\resources\\bin\\orca.exe'")
  })
