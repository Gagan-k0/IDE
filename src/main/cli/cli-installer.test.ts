...
describe('CliInstaller', () => {
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const appImagePath = join(fixture.root, 'LogicMantra.AppImage')
  ...
  it.skipIf(process.platform === 'win32')(
    ...
    async () => {
      ...
      const newAppImagePath = join(fixture.root, 'LogicMantra.AppImage')
  ...
  it('creates a windows wrapper and updates the user PATH', async () => {
    const fixture = await makeFixture()
    const installPath = join(fixture.root, 'Programs', 'LogicMantra', 'bin', 'orca.cmd')
    let userPath = 'C:\\Windows\\System32'
    const installer = new CliInstaller({
      platform: 'win32',
      isPackaged: false,
      userDataPath: fixture.userDataPath,
      execPath: 'C:\\Users\\me\\AppData\\Local\\LogicMantra\\LogicMantra.exe',
      appPath: fixture.appPath,
      commandPathOverride: installPath,
    ...
    expect(installed.state).toBe('installed')
    expect(installed.pathConfigured).toBe(true)
    expect(userPath).toContain(join(fixture.root, 'Programs', 'LogicMantra', 'bin'))
    ...
    const removed = await installer.remove()
    expect(removed.state).toBe('not_installed')
    expect(userPath).not.toContain(join(fixture.root, 'Programs', 'LogicMantra', 'bin'))
  })
  ...
  it('skips the Windows PATH write when removing an absent entry', async () => {
    ...
    const installer = new CliInstaller({
      ...
      execPath: 'C:\\Users\\me\\AppData\\Local\\LogicMantra\\LogicMantra.exe',
      appPath: fixture.appPath,
      commandPathOverride: join(fixture.root, 'Programs', 'LogicMantra', 'bin', 'orca.cmd'),
      userPathReader: async () => userPathRead('C:\\Windows\\System32'),
      userPathWriter
    })
  ...
  it('matches expandable Windows PATH entries case-insensitively without rewriting them', async () => {
    const fixture = await makeFixture()
    const installPath = join(fixture.root, 'Local App Data', 'LogicMantra', 'bin', 'orca.cmd')
    const userPathWriter = vi.fn()
    const installer = new CliInstaller({
      platform: 'win32',
      isPackaged: false,
      ...
      userPathReader: async () => userPathRead('%localappdata%\\LogicMantra\\bin\\', true),
      userPathWriter
    })
  ...
  it('does not expand environment variables stored in a REG_SZ Windows PATH', async () => {
    ...
    const installer = new CliInstaller({
      ...
      commandPathOverride: installPath,
      windowsEnvironment: { LOCALAPPDATA: join(fixture.root, 'Local App Data') },
      userPathReader: async () => userPathRead('%LOCALAPPDATA%\\LogicMantra\\bin'),
      userPathWriter
    })

    await installer.install()

    expect(userPathWriter).toHaveBeenCalledWith(`%LOCALAPPDATA%\\LogicMantra\\bin;${pathDirectory}`)
  })
  ...
  it.skipIf(process.platform === 'win32')(
    'replaces stale packaged LogicMantra launcher symlinks',
    async () => {
      const fixture = await makeFixture()
  ...
  it('resolves custom-install packaged Windows command path from resourcesPath', async () => {
    ...
    const resourcesPath = join(fixture.root, 'D Custom LogicMantra', 'resources')
    await mkdir(join(resourcesPath, 'bin'), { recursive: true })
    ...
    const installer = new CliInstaller({
      ...
      localAppDataPath,
      userDataPath: fixture.userDataPath,
      execPath: join(fixture.root, 'D Custom LogicMantra', 'LogicMantra.exe'),
      appPath: fixture.appPath,
  ...
  it('keeps a bundled Windows launcher installed when the user PATH read is unknown', async () => {
    ...
    const installer = new CliInstaller({
      ...
      resourcesPath,
      userDataPath: fixture.userDataPath,
      execPath: join(fixture.root, 'LogicMantra.exe'),
      appPath: fixture.appPath,
      userPathReader: async () => ({
        state: 'unknown',
        detail: 'LogicMantra could not read the Windows user PATH registry value.'
