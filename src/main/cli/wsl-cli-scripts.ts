const MANAGED_MARKER = '# LogicMantra managed WSL CLI launcher'
const BRIDGE_MANAGED_MARKER = '# LogicMantra managed WSL CLI PowerShell bridge'
...
try {
  ...
  if ($args.Count -lt 1) {
    throw 'Invalid LogicMantra WSL CLI bridge invocation.'
  }
  ...
  if ($null -eq $Process) {
    throw 'Unable to start the LogicMantra Windows CLI launcher.'
  }
  $Process.WaitForExit()
