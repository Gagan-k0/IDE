...
function buildManifest(tag: string): string {
  ...
  return [
    `version: ${version}`,
    'files:',
    `  - url: LogicMantra-${version}-arm64-mac.zip`,
    '    sha512: test',
    `path: LogicMantra-${version}-arm64-mac.zip`
  ].join('\n')
}
          ...
          text: () =>
            Promise.resolve(
              [
                ...
                '    sha512: test',
                `  - url: LogicMantra-${version}-mac.zip`,
                '    sha512: test',
                `path: LogicMantra-${version}-mac.zip`
              ].join('\n')
            )
        })
      }
          ...
          text: () =>
            Promise.resolve(
              [
                ...
                '  - url: orca-windows-setup.exe',
                '    sha512: test',
                '  - url: LogicMantra-1.4.28-mac.zip',
                '    sha512: test'
              ].join('\n')
            )
        })
