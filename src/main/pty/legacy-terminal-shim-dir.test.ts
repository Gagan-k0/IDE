...
describe('legacy terminal shim neutralization', () => {
  ...
  it('atomically replaces the legacy command paths with executable tombstones', () => {
    ...
    for (const path of [
      ...
      expect(readFileSync(path, 'utf8')).not.toContain('Co-authored-by: LogicMantra')
  ...
  itOnPosix('keeps the POSIX wrapper on a host where no absolute bash resolves', async () => {
    ...
    try {
      ...
      expect(git.split('\n')[0]).toBe('#!/bin/bash')
      expect(git).toContain('LogicMantra compatibility wrapper could not locate')
  ...
  itOnPosix('keeps a real Bash command hash working with trailing PATH separators', async () => {
    ...
    const child = spawn('bash', ['--noprofile', '--norc'], {
      ...
      env: {
        ...
        ORCA_GIT_COMMIT_TRAILER: 'Co-authored-by: LogicMantra <help@stably.ai>',
    ...
    expect(stdout).not.toContain('Co-authored-by: LogicMantra')
  ...
  it('strips legacy entries from every Windows PATH spelling', () => {
    const env: Record<string, string> = {
      PATH: 'C:\\LogicMantra\\orca-terminal-attribution\\win32',
      Path: 'C:\\LogicMantra\\orca-terminal-attribution\\win32;C:\\Windows\\System32'
    }
  ...
  it('matches a re-cased Windows shim path', () => {
    const env: Record<string, string> = {
      Path: 'C:\\LogicMantra\\LogicMantra-Terminal-Attribution\\Win32;C:\\Windows\\System32'
    }
