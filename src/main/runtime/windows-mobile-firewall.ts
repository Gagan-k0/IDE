...
const FIREWALL_RULE_NAME = 'LogicMantra.MobilePairing'
const FIREWALL_RULE_DISPLAY_NAME = 'LogicMantra Mobile Pairing'
...
New-NetFirewallRule -Name ${quotePowerShell(FIREWALL_RULE_NAME)} -DisplayName ${quotePowerShell(FIREWALL_RULE_DISPLAY_NAME)} -Description 'Allows LogicMantra Mobile to connect to this LogicMantra desktop on private networks.' -Direction Inbound -Action Allow -Enabled True -Profile Private -Protocol TCP -LocalPort ${port} -Program ${quotePowerShell(executablePath)} -EdgeTraversalPolicy Block | Out-Null`
}
