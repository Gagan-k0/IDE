...
describe('runtime RPC startup failure reporting', () => {
  ...
  it('shows the CLI impact and local cause', async () => {
    ...
    expect(showMessageBoxMock).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        type: 'error',
        title: 'LogicMantra CLI unavailable',
        message: "LogicMantra couldn't start its local command transport.",
  ...
  it.each([
    ['EACCES', "Check permissions on LogicMantra's data folder"],
    ['EPERM', "Check permissions on LogicMantra's data folder"],
    ...
    ['ENOENT', "LogicMantra's data folder may be missing"],
  ...
  it('falls back to a plain restart when the cause is unrecognised', async () => {
    ...
    const detail = showMessageBoxMock.mock.calls[0]?.[1]?.detail as string
    expect(detail).toContain('Restart LogicMantra to try again.')
    expect(detail).not.toContain("Check permissions on LogicMantra's data folder")
  })
