...
describe('createManagedCommandMatcher', () => {
  ...
  it('normalizes Windows backslashes so cmd-style paths still match', () => {
    expect(match('C:\\Users\\alice\\AppData\\Roaming\\LogicMantra\\agent-hooks\\claude-hook.sh')).toBe(
      true
    )
  })
  ...
  it('removes managed direct bash/powershell/command fields', () => {
    const cleaned = removeManagedCommands(
      [
        ...
        {
          type: 'command',
          powershell: "& 'C:\\Users\\alice\\LogicMantra\\agent-hooks\\copilot-hook.sh'",
          timeoutSec: 5
        },
        {
          type: 'command',
...
describe('getSharedManagedScriptPath', () => {
  ...
  it('does not depend on Electron app.getPath, so two LogicMantra instances resolve to the same path', () => {
