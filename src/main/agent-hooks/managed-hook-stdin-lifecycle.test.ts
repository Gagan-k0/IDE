...
describe('Windows managed hook stdin structure', () => {
  it('exits immediately when LogicMantra env is missing and keeps drain for other failures', () => {
  ...
  it.skipIf(process.platform !== 'win32')(
    'exits 0 for every local script and missing-script launcher, dropping stdin only without LogicMantra env',
    async () => {
...
describe.skipIf(process.platform === 'win32')('managed hook stdin lifecycle', () => {
  ...
  it('accepts a large payload without LogicMantra environment or a broken writer', async () => {
    const scripts = await generatePosixScripts()
    for (const [agent, script] of scripts) {
