...
describe('ensureRealHomeCodexHookState (install)', () => {
  it('creates hooks.json with the LogicMantra entry in every managed event for a fresh home', () => {
    grantSucceeds()
  ...
  it("keeps later user handler trust positions stable inside LogicMantra's hook group", () => {
    grantSucceeds()
    ensureRealHomeCodexHookState({ hooksEnabled: true, userDataPath: userDataDir })
    const installed = readRealHooksJson()
...
describe('ensureRealHomeCodexHookState (opt-out sweep)', () => {
  it('rebases trust when a user appended hooks after LogicMantra installed', () => {
    grantSucceeds()
  ...
  it('removes only LogicMantra entries and reports the removed lane', () => {
    grantSucceeds()
    const userStop = {
      matcher: 'deploy-*',
  ...
  it('removes only hash-proven LogicMantra trust from a mixed hook group', () => {
    const material = getCodexManagedHookInstallMaterial()
