...
describe('CodexHookService', () => {
  it('installs PermissionRequest with trust so Codex approval prompts reach LogicMantra', () => {
  ...
  it('keeps hooks isolated by LogicMantra userData instead of mutating system ~/.codex', () => {
    const systemCodexHome = join(tmpHome, '.codex')
  ...
  it('refreshes runtime user hooks without installing LogicMantra-managed hooks', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(
  ...
  it('removes legacy LogicMantra-managed hooks from system ~/.codex during install', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
  ...
  it('removes very large legacy LogicMantra-managed hook lists from system ~/.codex', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const systemHooksPath = join(systemCodexHome, 'hooks.json')
    const legacyScriptPath = join(
      tmpHome,
  ...
  it('removes the legacy LogicMantra Codex profile file when it only contains managed hooks', () => {
    const systemCodexHome = join(tmpHome, '.codex')
  ...
  it('removes only the legacy LogicMantra block from a user-edited Codex profile file', () => {
    const systemCodexHome = join(tmpHome, '.codex')
    const profilePath = join(systemCodexHome, 'orca-agent-status.config.toml')
    mkdirSync(systemCodexHome, { recursive: true })
    writeFileSync(
