...
function assertHooksJsonGeneration(
  ...
  if (currentRaw !== expectedRaw || resolveHooksJsonWritePath(hooksJsonPath) !== hooksWritePath) {
    ...
    throw new Error('Codex hooks.json changed while LogicMantra prepared its trust repair')
  }
}
...
function sweepRealHomeCodexHook(): RealHomeCodexHookLane {
  ...
  if (removedAny) {
    ...
    try {
      ...
      console.warn('[codex-real-home-hooks] failed to drop LogicMantra trust entries:', error)
    }
  }
  return 'removed'
}
