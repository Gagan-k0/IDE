...
const AMP_PLUGIN_MARKER = 'Managed by LogicMantra. Do not edit; changes may be overwritten.'
  ...
  switch (state.kind) {
    ...
    case 'unmanaged':
      return {
        ...
        configPath: pluginPath,
        managedHooksPresent: false,
        detail: 'Amp LogicMantra status plugin exists but is not LogicMantra-managed'
      }
    case 'error':
      return {
        agent: 'amp',
...
function getAmpPluginSource(): string {
  return [
    ...
    '  // Why: Amp sessions can outlive a LogicMantra restart; the endpoint file is',
    ...
    '    })',
    '  } catch {',
    '    // Why: LogicMantra status reporting must never affect the Amp run.',
    '  } finally {',
    ...
    '  // payload closures when LogicMantra is down and each POST waits for timeout.',
