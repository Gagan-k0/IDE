...
describe('hosted review branch cache (#11532)', () => {
  ...
  it('retires a cached no-review answer when LogicMantra opens a review', async () => {
    const lookup = vi
      .fn<() => Promise<HostedReviewInfo | null>>()
  ...
  it('discards a lookup that was already in flight when LogicMantra opened a review', async () => {
    let resolveLookup: (value: HostedReviewInfo | null) => void = () => {}
    const lookup = vi
      .fn<() => Promise<HostedReviewInfo | null>>()
