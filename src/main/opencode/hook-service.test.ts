...
describe('OpenCodeHookService overlay mode (user OPENCODE_CONFIG_DIR set)', () => {
  ...
  it('builds an overlay under userData and exposes user config + LogicMantra plugin together', () => {
    const service = new OpenCodeHookService()
  ...
  it("does not overwrite a user plugin file with the same filename as LogicMantra's plugin", () => {
