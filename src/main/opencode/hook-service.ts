...
export function getOpenCodeFamilyPluginSource(hookPathname: string): string {
  ...
  return [
    ...
    "// OpenCode's Node process (not LogicMantra's) and has no access to server.ts's",
    ...
    '// which reads the endpoint file. The file only changes on LogicMantra restart',
    ...
    '      // writeEndpointFile on the LogicMantra side) allocates a fresh inode on',
    ...
    '  // frozen when OpenCode was fork()ed — stale after a LogicMantra restart. The',
    '  // file is rewritten on every LogicMantra start(), so sourcing it per post lets',
    '  // a long-running OpenCode session reach the current server. Falls back',
    '  // to process.env when the file is absent (first-run / pre-endpoint-file / LogicMantra',
    ...
    '// per turn through LogicMantra (loopback HTTP -> main JSON parse -> status compare',
    ...
    '// flip the root LogicMantra pane to done or overwrite the parent turn preview.',
    ...
    '  // rewritten by a newer LogicMantra since the last call. Pane/tab/worktree IDs',
    ...
    '    // because LogicMantra is unavailable or the local loopback request failed.',
    '    return false;',
    '  } finally {',
    '    clearTimeout(timeout);',
    '  }',
    ...
    '          // publish a final idle so LogicMantra does not retain a dead owner.',
