...
describe('captureLinuxPackageArtifact', () => {
  ...
  it('accepts an uppercase extension', () => {
    capture({ downloadedFile: path.join(downloadDir, 'LogicMantra.DEB') })
    expect(recovery.getTrackedLinuxPackageArtifact()?.path).toBe(path.join(downloadDir, 'LogicMantra.DEB'))
  })
  ...
  it('matches a percent-encoded relative URL against the decoded basename', () => {
    const downloadedFile = path.join(downloadDir, 'LogicMantra Setup 1.2.3.deb')
    capture({ downloadedFile, files: [{ url: 'LogicMantra%20Setup%201.2.3.deb', sha512: SHA512 }] })
