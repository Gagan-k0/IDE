...
function createWebPreloadApi(): Partial<PreloadApi> {
  const webOrcaProfileAuthStatus = () =>
    Promise.resolve({
      ...
      setupMessage: 'LogicMantra Cloud sign-in is not available in the browser fallback.'
    })
...
function createRuntimeEnvironmentsApi(): NonNullable<Partial<PreloadApi>['runtimeEnvironments']> {
  return {
    ...
    addFromPairingCode: async ({ name, pairingCode }) => {
      ...
      if (!offer) {
        throw new Error('Invalid LogicMantra pairing code.')
      }
      ...
      try {
        ...
        return {
          ...
          message: translate(
            ...
            'Cannot reach LogicMantra at {{endpoint}}.',
      ...
      try {
        ...
        return {
          ...
          message: translate(
            ...
            'LogicMantra verified the host but could not save it. Check browser storage and try again.'
          )
        }
      }
      manuallyDisconnectedEnvironmentIds.clear()
...
function createCliApi(): NonNullable<Partial<PreloadApi>['cli']> {
  const status = {
    ...
    detail: 'CLI registration is managed on the LogicMantra server, not in the web browser.'
  } as const
  return {
...
function createAgentHooksApi(): NonNullable<Partial<PreloadApi>['agentHooks']> {
  const status = (
    ...
    Promise.resolve({
      ...
      detail: 'Agent hook status is only available on the LogicMantra server.'
    } as const)
  return {
  ...
  return {
    ...
    openSetup: (args) =>
      callRuntimeResult<ComputerUsePermissionSetupResult>(
        ...
        nextStep: 'Computer-use permissions are managed on the LogicMantra server.'
      })),
    reset: () =>
      Promise.resolve({
        platform: getBrowserPlatform(),
...
function resolveEnvironment(selector: string): StoredWebRuntimeEnvironment {
  ...
  throw new Error(`Unknown LogicMantra runtime environment: ${selector}`)
}
...
function requireActiveEnvironment(): StoredWebRuntimeEnvironment {
  ...
  if (!activeEnvironment) {
    throw new Error('Pair this web client with a LogicMantra server first.')
  }
...
function assertActiveEnvironment(environmentId: string): void {
  if (requireActiveEnvironment().id !== environmentId) {
    throw new Error('The paired LogicMantra server changed while the request was in progress.')
  }
}
