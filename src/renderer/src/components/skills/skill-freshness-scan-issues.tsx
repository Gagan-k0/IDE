  ...
  switch (issue.reason) {
    case 'depth-limit':
      return translate(
        ...
        'LogicMantra reached its plugin scan depth limit before checking this folder.'
      )
    case 'entry-limit':
      return translate(
        'auto.components.skills.SkillFreshnessUpdateDialog.scanEntryLimit',
        'LogicMantra reached its plugin scan entry limit before checking the rest of this cache.'
      )
    case 'candidate-limit':
      return translate(
        'auto.components.skills.SkillFreshnessUpdateDialog.scanCandidateLimit',
        'LogicMantra found more same-named skill folders than it can safely inspect.'
      )
    case 'manifest-limit':
      return translate(
        'auto.components.skills.SkillFreshnessUpdateDialog.scanManifestLimit',
        'LogicMantra skipped this plugin manifest because it exceeded a safe limit.'
      )
    case 'outside-root':
      return translate(
        'auto.components.skills.SkillFreshnessUpdateDialog.scanOutsideRoot',
        'LogicMantra skipped this plugin path because it points outside the plugin cache.'
      )
    case 'io-error':
      return issue.errorCode
        ? translate(
            'auto.components.skills.SkillFreshnessUpdateDialog.scanIoErrorWithCode',
            'LogicMantra could not read this plugin path ({{value0}}).',
            { value0: issue.errorCode }
          )
        : translate(
            'auto.components.skills.SkillFreshnessUpdateDialog.scanIoError',
            'LogicMantra could not read this plugin path.'
          )
    case 'issue-limit':
      return translate(
        'auto.components.skills.SkillFreshnessUpdateDialog.scanIssueLimit',
        'LogicMantra found too many skipped plugin folders to list individually.'
      )
  }
}
