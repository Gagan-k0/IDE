...
describe('filterAiVaultSessions', () => {
  ...
  it('filters project scope by the resolved active project key', () => {
    ...
    const sessionProjectById = new Map([
      [projectSession.id, { kind: 'repo' as const, key: 'project:orca', label: 'LogicMantra' }],
  ...
  it('matches repo: queries against resolved project labels before folder fallback', () => {
    const sessionProjectById = new Map([
      [baseSession.id, { kind: 'repo' as const, key: 'project:orca', label: 'Canonical LogicMantra' }]
    ])
    const projectLabelByKey = new Map([['project:orca', 'Canonical LogicMantra']])
  ...
  it('groups sibling worktree sessions by project label when resolved', () => {
    ...
    const sessionProjectById = new Map(
      sessions.map((session) => [
        session.id,
        { kind: 'repo' as const, key: 'project:orca', label: 'LogicMantra' }
      ])
    )
    const projectLabelByKey = new Map([['project:orca', 'Canonical LogicMantra']])
    ...
    ).toEqual([{ key: 'project:orca', label: 'Canonical LogicMantra', sessions }])
  })
