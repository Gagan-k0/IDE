        ...
        <div className="space-y-0.5">
          ...
          <p className="text-xs text-muted-foreground">
            {translate(
              ...
              'Let coding agents control the active mobile emulator with LogicMantra CLI commands.'
            )}
          </p>
        </div>
        <span
          ...
          <div className="min-w-0 flex-1 space-y-1">
            <p className="text-sm font-medium">
              {translate(
                ...
                'Enable LogicMantra CLI'
              )}
            </p>
            <p className="text-xs text-muted-foreground">
              {translate(
                'auto.components.settings.MobileEmulatorAgentControlRow.2fef055608',
                'Registers the LogicMantra CLI command so agents can control the active emulator from their shell.'
              )}
            </p>
            ...
            {setup.cliBusy ? (
              <p className="text-[11px] leading-snug text-muted-foreground">
                {translate(
                  ...
                  'Registering the LogicMantra CLI'
                )}{' '}
                {setup.cliInstallStatus?.commandPath ? (
                  <code className="rounded bg-muted px-1 py-0.5">
                    {setup.cliInstallStatus.commandPath}
        ...
        <div className={cn('py-3', setup.step2Blocked && 'opacity-60')}>
          <AgentSkillSetupPanel
            ...
            title={translate(
              ...
              'LogicMantra CLI skill'
            )}
            description={translate(
              'auto.components.settings.MobileEmulatorAgentControlRow.d94ca6a623',
              'Enables agents to use LogicMantra CLI commands, including mobile emulator control.'
            )}
            command={cliSkillInstallCommand}
            installedCommand={cliSkillUpdateCommand}
            terminalTitle="LogicMantra CLI skill setup"
            terminalAriaLabel="LogicMantra CLI skill install terminal"
            ...
            openingHint={translate(
              ...
              'Checking LogicMantra CLI before opening skill setup.'
            )}
