...
describe('browser settings search copy', () => {
  ...
  it('keeps the pre-feature wording while inverting is off', () => {
    expect(getBrowserLinkRoutingDescription({ isMac: true })).toBe(
      "Open http(s) links in LogicMantra's built-in browser — from the terminal, markdown, and the editor. ⇧⌘-click always uses your system browser."
    )
...
describe('browser link routing modifier copy', () => {
  ...
  it('keeps the search entry order BrowserPane indexes by position', () => {
    expect(getBrowserPaneSearchEntries({ isMac: true }).map((entry) => entry.title)).toEqual([
      ...
      'Default Zoom',
      'Link Routing',
      'Hold Shift to open in LogicMantra',
  ...
  it('names the destination the modifier actually reaches', () => {
    expect(getLinkRoutingModifierTitle(false)).toBe('Hold Shift to open in LogicMantra')
  ...
  it('points the description at LogicMantra only when links currently open externally', () => {
    expect(getLinkRoutingModifierDescription({ openLinksInApp: false, isMac: true })).toContain(
      "LogicMantra's built-in browser"
    )
  ...
  it('phrases the LogicMantra branch as enabled-state copy', () => {
    expect(getLinkRoutingModifierDescription({ openLinksInApp: false, isMac: true })).toContain(
      'When enabled'
    )
  })
...
describe('Link Routing description localization', () => {
  ...
  it('renders the Korean copy with the shortcut interpolated', async () => {
    ...
    expect(description).not.toContain("LogicMantra's built-in browser")
    ...
    await i18n.changeLanguage('en')
    expect(getBrowserLinkRoutingDescription({ isMac: true })).toContain("LogicMantra's built-in browser")
  })
