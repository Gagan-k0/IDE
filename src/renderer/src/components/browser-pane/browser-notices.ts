...
export function formatPermissionNotice(event: BrowserPermissionDeniedEvent): string {
  ...
  return `${target} asked for ${humanizePermission(event.permission)}, and LogicMantra denied it.`
}
...
export function formatPopupNotice(event: BrowserPopupEvent): string {
  ...
  if (event.action === 'opened-in-orca') {
    return `${target} opened a new page in LogicMantra.`
  }
  if (event.action === 'opened-external') {
    return `${target} opened a new window in your default browser.`
  }
  return `${target} tried to open a popup LogicMantra does not support here.`
}
  ...
  if (isChromiumCertificateErrorCode(loadError.code)) {
    ...
    if (loadError.code === -202) {
      return translate(
        ...
        "LogicMantra doesn't trust the authority that issued the certificate for {{value0}}.",
        { value0: host }
      )
    }
    return translate(
      'browser.loadFailure.certificateVerificationFailed',
      "LogicMantra couldn't verify the certificate for {{value0}}.",
      { value0: host }
    )
  }
