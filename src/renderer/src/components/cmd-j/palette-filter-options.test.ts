...
const repos = [repo('r1', 'LogicMantra'), repo('r2', 'LogicMantra (builder)', 'ssh-1'), repo('r3', 'Solo')]
const projects = [project('p1', 'LogicMantra')]
...
describe('buildPaletteFilterModel', () => {
  ...
  it('sorts project rows by workspace count then label', () => {
    ...
    expect(model.projects.map((option) => option.label)).toEqual(['LogicMantra', 'Solo'])
  })
