  ...
  return {
    ...
    descriptionSuffix: args.isBatchDelete
      ? allFolderWorkspaceDeletes
        ? 'from LogicMantra. Project folders on disk will not be deleted.'
        : mixedFolderWorkspaceDeletes
          ? 'from LogicMantra. Git worktrees will also be removed from git and disk; folder workspaces will only remove the LogicMantra workspace entry.'
          : 'from git and delete their workspace folders.'
      : args.isFolderWorkspaceDelete
        ? 'from LogicMantra. The project folder on disk will not be deleted.'
