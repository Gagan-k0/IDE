...
describe('connectPanePty', () => {
  ...
  it('surfaces an actionable error when a Codex backfill timeout drops to the shell', async () => {
    ...
    expect(deps.onPtyErrorRef.current).toHaveBeenCalledWith(
      1,
      expect.stringContaining('LogicMantra attempts background recovery for managed local and WSL homes')
    )
  })
  ...
  it('clears pre-hook launch config when a LogicMantra-started command exits', async () => {
    vi.useFakeTimers({ toFake: ['setTimeout'] })
    const { connectPanePty } = await import('./pty-connection')
  ...
  it('confirms a LogicMantra-launched Droid fresh spawn in a no-OSC shell (Git Bash)', async () => {
  ...
  it('retries null remote snapshots for overflowed hidden runtime output', async () => {
    ...
    expect(pane.terminal.write).not.toHaveBeenCalledWith(
      expect.stringContaining('LogicMantra skipped hidden terminal output'),
      expect.any(Function)
    )
    expect(pane.terminal.write).not.toHaveBeenCalledWith(
    ...
    try {
      ...
      expect(pane.terminal.write).toHaveBeenCalledWith(
        expect.stringContaining(
          'LogicMantra skipped hidden terminal output because main recovery was unavailable.'
        ),
        expect.any(Function)
      )
      expect(pane.terminal.write).toHaveBeenCalledWith(
