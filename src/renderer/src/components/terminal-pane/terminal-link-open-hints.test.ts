...
describe('getTerminalUrlOpenHint', () => {
  ...
  it('keeps the system-browser wording when inverting but links open in LogicMantra', () => {
    stubPlatform(true)
  ...
  it('names LogicMantra when inverting and links open externally', () => {
    stubPlatform(true)
    expect(getTerminalUrlOpenHint({ openLinksInApp: false, modifierInverts: true })).toBe(
      'Click for actions, ⌘+click to open, or ⇧⌘+click to open in LogicMantra'
    )
  })
  ...
  it('uses the Ctrl chord off macOS', () => {
    stubPlatform(false)
    expect(getTerminalUrlOpenHint({ openLinksInApp: false, modifierInverts: true })).toBe(
      'Click for actions, Ctrl+click to open, or Shift+Ctrl+click to open in LogicMantra'
    )
  })
  ...
  it('omits the action-menu gesture when terminal link actions are disabled', () => {
    ...
    ).toBe('Ctrl+click to open, or Shift+Ctrl+click to open in LogicMantra')
  })
})
...
describe('terminalUrlOpenHintOptionsFor', () => {
  ...
  it('keeps inversion for a local pane while a remote runtime is active', () => {
    ...
    expect(getTerminalUrlOpenHint(options)).toContain('to open in LogicMantra')
  })

  it('keeps inversion for a runtime pane when its host can open a LogicMantra browser', () => {
    const options = terminalUrlOpenHintOptionsFor(
      {
        openLinksInApp: false,
        openLinksInAppModifierInverts: true
