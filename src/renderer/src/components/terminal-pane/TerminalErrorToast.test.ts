...
beforeEach(() => {
  ...
  environmentMocks.resolveFooter.mockResolvedValue(
    ['---', 'LogicMantra: 1.4.178-rc.2', 'OS: darwin 25.0.0 (arm64)', 'Shell: /bin/zsh'].join('\n')
  )
})
...
describe('isExplainedTerminalError', () => {
  ...
  it('keeps the issue link for errors LogicMantra cannot explain', () => {
...
describe('shouldOfferDaemonRestart', () => {
  ...
  it('matches stale daemon cwd failures', () => {
    expect(
      shouldOfferDaemonRestart(
        "Daemon's working directory is gone (worktree deleted?). Restart LogicMantra. node-pty: daemon_cwd failed: ENOENT (errno 2, No such file or directory) - cwd='<unavailable>'"
      )
    ).toBe(true)
  })
...
describe('TerminalErrorToast environment footer', () => {
  it('appends client environment details to local errors', async () => {
    ...
    await waitFor(() => expect(view.container.textContent).toContain('LogicMantra: 1.4.178-rc.2'))
  })
  ...
  it('does not retain a prior async footer when the next error already has one', async () => {
    ...
    expect(view.container.textContent).toContain('LogicMantra: embedded')
    expect(view.container.textContent).not.toContain('LogicMantra: 1.4.178-rc.2')
  })
