...
const SUMMARY = 'LogicMantra downloaded the update but could not install the system package automatically.'
const COPIED_NOTE =
  `Command copied. Run it in a system terminal to install ${PACKAGE_FILE_NAME}, ` +
  'then quit and reopen LogicMantra.'
const INSTRUCTIONS = {
  ok: true as const,
...
describe('LinuxPackageInstallRecoveryCard details', () => {
  ...
  it('scopes the trust note to the moment the command is built', () => {
    ...
    expect(detail.textContent).toContain(
      'The system package itself is not signature-checked, and LogicMantra cannot vouch for the file ' +
        'after that point.'
    )
  })
