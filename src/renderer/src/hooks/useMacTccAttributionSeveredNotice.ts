      ...
      try {
        ...
        toast.warning(
          translate(
            ...
            'macOS permissions may not reach LogicMantra terminals'
          ),
          {
            id: SEVERED_TCC_NOTICE_ID,
            description: translate(
              'auto.hooks.useMacTccAttributionSeveredNotice.description',
              'Running LogicMantra terminals are hosted by a daemon started by a previous LogicMantra installation. macOS may not apply LogicMantra’s Accessibility, Automation, or protected-file permissions to them. Restart the daemon from Manage Sessions to restore access. This will close all running LogicMantra terminals.'
            ),
            duration: Infinity,
            action: {
              label: translate(
