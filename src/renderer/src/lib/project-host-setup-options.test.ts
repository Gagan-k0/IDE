...
describe('buildProjectHostSetupOptions', () => {
  it('returns ready setup choices for one project sorted with local first', () => {
    const options = buildProjectHostSetupOptions({
      ...
      hosts: [host('local'), host('ssh:builder')],
      projectHostSetups: [
  ...
  it('omits ready setups whose host is no longer actionable', () => {
    const options = buildProjectHostSetupOptions({
      projectId: 'project-1',
      eligibleRepos: [repo('local-repo'), repo('removed-repo')],
      hosts: [host('local')],
      projectHostSetups: [
        setup('local', 'project-1', 'local', 'local-repo'),
        setup('removed', 'project-1', 'ssh:removed', 'removed-repo')
      ]
    })

    expect(options.map((option) => option.id)).toEqual(['local'])
    expect(options).not.toContainEqual(
      expect.objectContaining({ connectAction: { kind: 'ssh', targetId: 'removed' } })
    )
  })
  ...
  it('marks incompatible runtime hosts as visible but unavailable', () => {
    ...
    expect(options).toEqual([
      ...
      expect.objectContaining({
        ...
        kind: 'needs-setup',
        label: 'GPU VM',
        detail: 'LogicMantra server version is incompatible',
        isAvailable: false
      })
    ])
  })
  ...
  it('marks runtime hosts without project setup capability as unavailable', () => {
    ...
    expect(options.at(-1)).toMatchObject({
      id: 'needs-setup:runtime:gpu',
      kind: 'needs-setup',
      detail: 'Update LogicMantra on this host to set up projects',
      isAvailable: false
    })
  })
