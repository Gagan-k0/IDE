...
describe('openFileInBrowserTab', () => {
  it('opens a local file URL in the LogicMantra browser with the filename as title', () => {
    openFileInBrowserTab({
  ...
  it('reports a paired-runtime recovery failure without an unhandled rejection', async () => {
    ...
    await vi.waitFor(() =>
      expect(mocks.toastError).toHaveBeenCalledWith('Unable to open this file in LogicMantra Browser.')
    )
