...
export const AGENT_SKILL_CLI_PREREQUISITE_NOTICE =
  'Before opening setup, LogicMantra may show a system prompt to register the LogicMantra CLI command on PATH.'

export const CLI_PREREQUISITE_REGISTRATION_TOAST = 'LogicMantra needs to register its CLI on PATH.'
export const CLI_PREREQUISITE_REGISTRATION_TOAST_DESCRIPTION =
  'Approve the system prompt so skill setup can use the LogicMantra CLI command.'
    ...
    toast.error(
      error instanceof Error
        ...
        : translate(
            ...
            'Failed to register the LogicMantra CLI in PATH.'
          )
    )
    return null
  }
  ...
  if (!status.supported) {
    toast.warning(
      translate(
        ...
        'LogicMantra CLI registration is unavailable'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            'Install the LogicMantra CLI before running agent skill setup.'
          )
      }
    )
    return
  ...
  if (status.state !== 'installed') {
    toast.warning(
      translate(
        ...
        'LogicMantra CLI registration needs attention'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.pathConfigured === null) {
    toast.warning(
      translate(
        ...
        'LogicMantra could not check your Windows user PATH'
      ),
      {
        description:
          status.detail ??
  ...
  if (status.pathConfigured === false) {
    ...
    toast.warning(
      translate(
        ...
        'LogicMantra CLI is not visible on PATH yet'
      ),
      {
        description:
          status.detail ??
          translate(
            ...
            'Restart your shell or add the LogicMantra CLI directory to PATH before setup.'
          )
      }
    )
  }
