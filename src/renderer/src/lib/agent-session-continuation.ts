...
export function buildAgentSessionContinuationPrompt(
  ...
  const sourceLines = [
    ...
    source.sourceLabel ? `LogicMantra pane: ${source.sourceLabel}` : null,
  ...
  return [
    'Continue work from the prior LogicMantra session using the context below.',
    ...
    ...(statusHints.length > 0 ? ['', 'Latest LogicMantra status hints:', ...statusHints] : []),
    '',
